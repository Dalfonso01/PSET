...
def forward_backward(all_possible_hidden_states,
    ...
    for i in range(num_time_steps - 1):
        message_pre_aft = robot.Distribution()
        for state in forward_messages[i]:
            edge_pot_xx = transition_model(state)
            if observations[i] is not None:
                edge_pot_yx = observation_model(state)[observations[i]]
                for state1 in edge_pot_xx:
                    message_pre_aft[state1] += edge_pot_xx[state1]*edge_pot_yx*forward_messages[i][state]
            else:
                for state1 in edge_pot_xx:
                    message_pre_aft[state1] += edge_pot_xx[state1]*forward_messages[i][state]
        print("bbb",message_pre_aft)
        message_pre_aft.renormalize()
        forward_messages[i+1] = message_pre_aft
    ...
    for hidden_state in all_possible_hidden_states:
        backward_messages[-1][hidden_state] = 1
    
    
    # TODO: Compute the backward messages
    for i in reversed(range(1,num_time_steps)):
        message_aft_pre = robot.Distribution()
        for state_pre in all_possible_hidden_states:
            edge_pot_xx = transition_model(state_pre)
            if observations[i] is not None:
                edge_pot_yx = observation_model(state1)[observations[i]]
                for state1 in backward_messages[i]:
                    message_aft_pre[state_pre] += edge_pot_xx[state1]*edge_pot_yx*backward_messages[i][state1]
            else:
                for state1 in backward_messages[i]:
                    message_aft_pre[state_pre] += edge_pot_xx[state1]*backward_messages[i][state1]
        print("aaa",message_aft_pre)            
        message_aft_pre.renormalize()     
        backward_messages[i-1] = message_aft_pre
    ...
    for i in range(num_time_steps):
        ...
        for state in forward_messages[i]:
            if state in backward_messages[i]:
                if observations[i] is not None:
                    edge_pot_yx = observation_model(state)[observations[i]]
                    marginal[state] = forward_messages[i][state]*backward_messages[i][state]*edge_pot_yx
                else:
